...
def rebalanceMinSizeDeltaAsset(_amountAsset, _priceAsset, _orderSizeAssets, _weightAssets, _TBDAmount):
    ...
    for i in range(nbAsset):
        _rebalanceMinSizeDelta[i] = _rebalanceDelta[i] if (
            abs(_rebalanceDelta[i]) < _orderSizeAssets[i, 0]) else 0
...
def rawCapFilledAsset(_amountAsset, _priceAsset, _orderSizeAssets,
    ...
    for i in range(nbAsset):
        list = np.where(_assetCapRank < _assetCapRank[i])[0]
        rawCapFilled[i] = np.sum(_minMaxCurrentDif[list])
...
def capToFillAsset(_amountAsset, _priceAsset, _orderSizeAssets, _weightAssets,
    ...
    _withdrawTerm = _withdrawIND * np.maximum(
         _minMaxCurrentDif, (_TBDAmount - _rebalanceDeltaTotal - np.maximum(
            (_TBDAmount - _rebalanceDeltaTotal), _rawCapFilled)))
    return _depositTerm + _withdrawTerm


def boundCapToFillasset(_amountAsset, _priceAsset, _orderSizeAssets, _weightAssets,
                   _TBDAmount):
    _depositIND = depositINDAsset(_TBDAmount)
    ...
    _depositTerm = _depositIND * np.minimum (_TBDAmount + _rebalanceMinSizeDeltaTotal , _rawCapFilled)
    _withdrawIND = withdrawINDAsset(_TBDAmount)
    _rebalanceDeltaTotal = rebalanceDeltaTotalAsset(_amountAsset, _priceAsset,
    
                                                    _weightAssets, _TBDAmount)
    _withdrawTerm = _withdrawIND * np.maximum(_TBDAmount - _rebalanceDeltaTotal,  _rawCapFilled)

    return _depositTerm + _withdrawTerm


def capInclusiveAsset(_amountAsset, _priceAsset, _orderSizeAssets, _weightAssets,
                   _TBDAmount):
    _depositIND = depositINDAsset(_TBDAmount)
    ...
    _boundCapToFillasset = boundCapToFillasset(_amountAsset, _priceAsset, _orderSizeAssets, _weightAssets,
                   _TBDAmount)

    _depositTerm = _depositIND * np.minimum (_TBDAmount , _minMaxCurrentDif + _boundCapToFillasset)
    _withdrawIND = withdrawINDAsset(_TBDAmount)
    _rebalanceDeltaTotal = rebalanceDeltaTotalAsset(_amountAsset, _priceAsset,
    
                                                    _weightAssets, _TBDAmount)
    _withdrawTerm = _withdrawIND * np.maximum(_TBDAmount - _rebalanceDeltaTotal , _minMaxCurrentDif + _boundCapToFillasset)
...
#    _capInclusiveAsset =  capInclusiveAsset(_amountAsset, _priceAsset,  #_orderSizeAssets, _weightAssets,
#                   _TBDAmount)

#    _depositTerm = _depositIND * np.minimum(
#        _minMaxCurrentDif,  _capInclusiveAsset - _boundCapToFillasset)
...
def orderSize(_amountAsset, _priceAsset, _orderSizeAssets, _weightAssets,
    ...
    _additionalOrders = additionalOrdersAsset(_amountAsset, _priceAsset,
                                              _orderSizeAssets, _weightAssets,
                                              _TBDAmount)
    _minOrderAsset = minOrderAsset( _amountAsset, _priceAsset, _orderSizeAssets, _weightAssets,
                  _TBDAmount)

    return _capToFillAsset / (_additionalOrders + 1) *  _minOrderAsset
